...
# 🪄 Custom table theme (with optional 2nd column highlight)
...
```{r build-eia-sep-report, include=FALSE, message=FALSE, warning=FALSE}
...
```{r setup-ntd-energy, include=FALSE}
DATA_DIR <- file.path("data", "mp02")
dir.create(DATA_DIR, showWarnings = FALSE, recursive = TRUE)

# ── 📥 Download NTD ENERGY File ──
...
# ── 📖 Read Raw Energy Data ──
NTD_ENERGY_RAW <- read_xlsx(NTD_ENERGY_FILE)

# Helper to replace NAs
...
# 🛠 Map Mode Codes to Full Names (NYC Subway Decoder)
...
# 🔁 Reshape to Long Format (for fuel analysis 🔍)
...
# 🧪 Preview Sample (like GTA radar blip)
...
# 🧼 Clean Raw Service Data (GTA-style)
...
```{r setup-automated, include=FALSE}
...
```{r setup-agency-size, include=FALSE}
...
# ⚡ Top 5 Electrified Agencies by Size
...
# ✅ Clean and shorten agency names
...
# Normalize CO₂ per mile for radar chart
...
-   The image was sourced by Chat GPT
